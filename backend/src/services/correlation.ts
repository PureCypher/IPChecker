...
export class CorrelationService {
  ...
  private getTrustRank(provider: string): number {
    const trustRanks: Record<string, number> = {
      'ip-api.com': parseInt(process.env.IPAPI_TRUST_RANK || '6'),
      'ipinfo.io': parseInt(process.env.IPINFO_TRUST_RANK || '8'),
      'ipdata.co': parseInt(process.env.IPDATA_TRUST_RANK || '7'),
      'abuseipdb.com': parseInt(process.env.ABUSEIPDB_TRUST_RANK || '9'),
      'shodan.io': parseInt(process.env.SHODAN_TRUST_RANK || '8'),
      'ipgeolocation.io': parseInt(process.env.IPGEOLOCATION_TRUST_RANK || '7'),
      'virustotal.com': parseInt(process.env.VIRUSTOTAL_TRUST_RANK || '9'),
      'threatminer.org': parseInt(process.env.THREATMINER_TRUST_RANK || '7'),
      'otx.alienvault.com': parseInt(process.env.ALIENVAULT_TRUST_RANK || '8'),
      'greynoise.io': parseInt(process.env.GREYNOISE_TRUST_RANK || '8'),
      'bgpview.io': parseInt(process.env.BGPVIEW_TRUST_RANK || '6'),
      // CTI Platforms
      'crowdsec.net': parseInt(process.env.CROWDSEC_TRUST_RANK || '9'),
      'ipqualityscore.com': parseInt(process.env.IPQUALITYSCORE_TRUST_RANK || '9'),
      'pulsedive.com': parseInt(process.env.PULSEDIVE_TRUST_RANK || '8'),
      'abuse.ch': parseInt(process.env.ABUSECH_TRUST_RANK || '9'),
      // Additional providers
      'torproject.org': parseInt(process.env.TORPROJECT_TRUST_RANK || '7'),
      'blocklist.de': parseInt(process.env.BLOCKLISTDE_TRUST_RANK || '8'),
      'cisco-talos.com': parseInt(process.env.CISCOTALOS_TRUST_RANK || '8'),
      'cins-army.com': parseInt(process.env.CINSARMY_TRUST_RANK || '8'),
      'spamhaus.org': parseInt(process.env.SPAMHAUS_TRUST_RANK || '9'),
      'malwarebazaar.abuse.ch': parseInt(process.env.MALWAREBAZAAR_TRUST_RANK || '9'),
      'ibm-xforce.com': parseInt(process.env.IBM_XFORCE_TRUST_RANK || '9'),
      'sans-isc.org': parseInt(process.env.SANSISC_TRUST_RANK || '8'),
      'vpnapi.io': parseInt(process.env.VPNAPI_TRUST_RANK || '8'),
      'proxycheck.io': parseInt(process.env.PROXYCHECK_TRUST_RANK || '8'),
      'iphub.info': parseInt(process.env.IPHUB_TRUST_RANK || '8'),
    };

    return trustRanks[provider] || 5;
  }
}
